...
abstract class DownloadMusic(private val mActivity: Activity) : IExecutor<Void?> {
    ...
    private fun checkNetwork() {
        val mobileNetworkDownload = MusicPreferences.enableMobileNetworkDownload
        if (com.blankj.utilcode.util.NetworkUtils.isMobileData() && !mobileNetworkDownload) {
    ...
    protected fun downloadMusic(url: String?, artist: String?, title: String?, coverPath: String?) {
        try {
            ...
            val musicAbsPath = FileUtils.getMusicDir() + "/" + fileName
